 ...
 */
@Gadget.ModulePrefs( //
title = "MassMob: Meetings and Smart Mobs in Apache Wave", //
author = "The MassMob development team", //
author_link = "http://massmob.ourproject.org", //
height = 640//, //
// Default width 320
// Commented only to use 100% width=550 (WAVE-309)//
)
@Gadget.InjectContent(files = { "ModuleContent.txt" })
public class MassmobGadget extends WaveGadget<UserPreferences> implements NeedsWave, EntryPoint,
  ...
  /** The gin. */
  protected MassmobGadgetDeployGinjector gin;
  ...
  private void createUIObjects() {
    gin = GWT.create(MassmobGadgetDeployGinjector.class);
    final WaveGINWrapper w = (WaveGINWrapper) gin.getWave();
    w.setWave(getWave());

    final MassmobMainPanel mainPanel = gin.getWaveGadget();
   ...
   *          the new height
   */
  private void setHeight(final MassmobMainPanel gadget) {
