...
public class UserConfirmBS { // extends ViewImpl implements UserConfirmView {
   ...
   *          the callback handler.
   */
  public native void confirm(String title, String msg, String acceptMsg, ConfirmCallback callback) /*-{
    $wnd.bootbox
        .dialog({
          message : msg,
          title : title,
  ...
  public void confirmAsk(final ConfirmAskEvent event) {
    ...
    if (TextUtils.notEmpty(event.getCancelBtnMsg())) {
      confirm(event.getTitle(), event.getMessage(), event.getAcceptBtnMsg(), event.getCancelBtnMsg(),
          callback);
    } else {
      confirm(event.getTitle(), event.getMessage(), event.getAcceptBtnMsg(), callback);
    }
  }
}
