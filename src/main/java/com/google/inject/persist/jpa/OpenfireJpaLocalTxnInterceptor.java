...
public class OpenfireJpaLocalTxnInterceptor implements MethodInterceptor {
  ...
  public Object invoke(final MethodInvocation methodInvocation) throws Throwable {
    ...
    final OpenfireTransactional transactional = readTransactionMetadata(methodInvocation);
  ...
  private OpenfireTransactional readTransactionMetadata(final MethodInvocation methodInvocation) {
    ...
    transactional = method.getAnnotation(OpenfireTransactional.class);
    if (null == transactional) {
      // If none on method, try the class.
      transactional = targetClass.getAnnotation(OpenfireTransactional.class);
    }
    if (null == transactional) {
      // If there is no transactional annotation present, use the default
      transactional = Internal.class.getAnnotation(OpenfireTransactional.class);
    }
