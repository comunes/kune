...
public class OpenRemoteServiceServlet extends HttpServlet
{
    ...
    public String processCall(String payload) throws SerializationException
    {
        ...
        // TODO(mmendez): need a way to check the type signature of the service
        // intf
        ...
        if (serviceIntfMethod==null)
        {
            // Bad payload, possible hack attempt.
            //
            throw new SecurityException("Method '"+methodName+"' (or a particular overload) on interface '"+serviceIntfName
        ...
        if (caught!=null)
        {
            handleException(responsePayload,caught);
        }
    ...
    private boolean isImplementedRemoteServiceInterface(String intfName)
    {
        synchronized (knownImplementedInterfaces)
        {
            ...
            // Unknown, so walk up the class hierarchy to find the first class
            // that
